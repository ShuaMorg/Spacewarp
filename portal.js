...
function createPortals(scene, renderer) {
  ...
  for (let i = 0; i < portalCoordinates.length; i++) {
    ...
    const renderTarget = new THREE.WebGLRenderTarget(128, 128);  // Adjust resolution as needed
    ...
    // Render the scene from the perspective of the portal camera into the render target only once
