...
function createPlanets(scene) {
  ...
  for (let i = 0; i < 5; i++) {  // Assume 5 suns for this example
    ...
    sun.position.set(
      Math.random() * 2000 - 1000,  // Spread out the suns more
    ...
    for (let j = 0; j < numPlanets; j++) {
      ...
      object.position.set(
        ...
        sun.position.z + (Math.random() - 0.5) * 20  // Random z position within a range
      );
      scene.add(object);
      objects.push(object);
    }
  ...
  for (let i = 0; i < 400; i++) {  // Assume 400 rogue planets for this example
    ...
    const object = new THREE.Mesh(objectGeometry, objectMaterial);
    object.position.set(
      Math.random() * 4000 - 2000,  // Spread out the rogue planets more
