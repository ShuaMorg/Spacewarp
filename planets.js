...
function updatePlanets(spacecraft, speed) {
  ...
  for (let object of objects) {
    ...
    if (spacecraft.position.distanceTo(object.position) < planetRadius + 0.5) {  // Adjusted collision detection distance
      const textureSrc = object.material.map.image.src;
      const planetTexture = textureSrc.substring(textureSrc.lastIndexOf('/') + 1);
      window.location.href = `view_planet.html?texture=${planetTexture}`;
    }
  }
}
